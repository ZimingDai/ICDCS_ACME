...
class pytorch_pruning(object):
    def __init__(self, parameters, pruning_settings=dict(), log_folder=None,id = -1,train_writer = None):
    ...
    def add_criteria(self):
        ...
        return copy.deepcopy(self.prune_network_accomulate["by_layer"])
    ...
    def compute_saliency(self,contributions):
        '''
        ...
        self.prune_network_accomulate["by_layer"] = contributions

        if validation_error > self.pruning_threshold:
            ## if error is big then skip pruning,如果误差较大，则跳过剪枝
        ...
        print('prune_neuron:{}/{}'.format(neuron_units,all_neuron_units))
        if self.train_writer is not None:
            self.train_writer.add_scalar(f'Client {self.id}/loss_neuron',
                                    self.all_neuron_units - self.neuron_units,self.iterations_done)
        ...
        # create groups per layer，按照层创建组
        ...
        # get an array of all criteria from groups，从组中获取所有标准的数组
        ...
        # adaptively estimate threshold given a number of neurons to be removed，自适应估计要删除的神经元数量的阈值
        ...
        self.log_debug = self.folder_to_write_debug + str(self.id)+'_debugOutput_pruning_%08d' % (
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            for group in groups[layer]:
                if group[1] <= threshold_now:
                    for unit in group[0]:
                        # do actual pruning   #这一步真正的执行剪枝操作
    ...
    def report_loss_neuron(self, training_loss, training_acc, train_writer = None, neurons_left = 0):
        ...
        if train_writer is not None:
            train_writer.add_scalar(f'Client {self.id}/loss_neuron', training_loss, self.all_neuron_units-self.neuron_units)
    ...
    def do_step(self, loss=None, optimizer=None, neurons_left=0, training_acc=0.0):
        ...
        # sets pruned weights to zero,确保在进行剪枝后，已被标记为剪枝的神经元的参数被设置为零
        self.enforce_pruning()

        # compute criteria for given batch
        self.importances = self.add_criteria()

        # small script to keep track of training loss since the last pruning,小脚本以跟踪自上次剪枝以来的训练损失
        self.util_add_loss(loss, training_acc)

        return copy.deepcopy(self.importances)

    def do_prune_step(self, optimizer, contributions,neurons_left=0):
        '''
        do one step of pruning,
        ...
        self.compute_saliency(contributions)  #该方法基于预先计算的标准值进行剪枝。需要在 add_criteria() 之后运行。
        self.set_momentum_zero_sgd(optimizer=optimizer) ##这个方法用于在使用 SGD 优化器的情况下，将已剪枝神经元的动量缓存清零；其目的是防止这些神经元在后续的训练过程中持续更新，从而保持其剪枝状态
