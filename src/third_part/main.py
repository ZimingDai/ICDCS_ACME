...
class FilteredCIFAR10(datasets.CIFAR10):
    def __init__(self, *args, target_classes=None, **kwargs):
        super(FilteredCIFAR10, self).__init__(*args, **kwargs)
        if target_classes is not None:
            # 获取需要的类别的索引
            indices = [i for i, target in enumerate(self.targets) if target in target_classes]
            # 过滤数据和标签
...
def main():
    ...
    parser.add_argument('--name', default='runs/test', type=str,
    ...
    parser.add_argument('--load_model', default='./runs/lenet3/lenet3_wo_prune/models/checkpoint.weights', type=str,
    ...
    parser.add_argument('--pruning-method', default=22, type=int,
    ...
    if args.dataset == "MNIST":
        ...
        #added by gf
        if args.rebulit_dataloader:
            train_loader = torch.utils.data.DataLoader(
                FilteredCIFAR10('./data', train=True, download=True, transform=transform_train,
            ...
            # 创建测试数据加载器
            test_loader = torch.utils.data.DataLoader(
                FilteredCIFAR10('./data', train=False, transform=transform_test, target_classes=[0,1,2]),
        ...
        traindir = os.path.join('/data/gaofei/data/imagenet/images', 'train')
        valdir = os.path.join('/data/gaofei/data/imagenet/images', 'val')
    ...
    #Creates a new network graph from pruned model, works with ResNet-101 only, "False"，这部分代码不是很懂
