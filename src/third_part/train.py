...
def load_and_cache_examples(feature_extractor, evaluate=False):
    transform = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ])
    dataset = datasets.CIFAR100(root='/data/gaofei/project/Taylor_pruning-master/data/cifar100', train=not evaluate, download=True, transform=transform)

    return dataset

def evaluate(args,model,feature_extractor):
    transform = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ])
    test_dataset = datasets.CIFAR100(root='/data/gaofei/project/Taylor_pruning-master/data/cifar100', train=False,
    ...
    model.eval()
    with torch.no_grad():
        for i, batch in enumerate(test_loader):  # 从数据加载器中迭代取出数据
...
def main():
    ...
    out_dir = '/data/gaofei/project/Taylor_pruning-master/runs/pre_ViTWithTempCNN1'
    out_txt = '/data/gaofei/project/Taylor_pruning-master/runs/pre_ViTWithTempCNN1/results.txt'
    ...
    with open(out_txt, 'a') as file_dir:
        # 准备模型、特征提取器和配置
        ...
        model = ViTWithTempCNN1.from_pretrained(args.model_dir, config=config, args=args, num_classes=100)
        ...
        # optimizer = optim.RAdam(filter(lambda p: p.requires_grad, model.parameters()), lr=5e-3)
        ...
        for epoch in range(args.epoch):  
            ...
            for i, batch in enumerate(train_loader):  # 从数据加载器中迭代取出数据
                ...
                # 将损失记录到TensorBoard
                writer.add_scalar(f'Train Loss', loss, all_step)
                all_step +=1

            # 在epoch结束时记录平均准确性
            main_accuracy = total_correct / total
            print(f"Train Epoch {epoch}, acc: {main_accuracy}")
            ...
            print(f"Test Epoch {epoch}, acc: {results}")

        # 加载最佳模型进行评估
        torch.save(best_model_state_dict, os.path.join(out_dir,'best_weights.pt'))
