...
@Time    :   2024/06/20 18:00:03
@Author  :   PhoenixDai
@Version :   1.0
@Site    :   http://phoenixdai.cn
@Desc    :   自定义了可变换长与宽的ViT模型, 将其覆盖Transformer库中的对应文件
"""
...
class ViTSelfAttention(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.query = DynaLinear(config.hidden_size, self.all_head_size, config.num_attention_heads, dyna_dim=[False, True], bias=config.qkv_bias)
        self.key = DynaLinear(config.hidden_size, self.all_head_size, config.num_attention_heads, dyna_dim=[False, True], bias=config.qkv_bias)
        self.value = DynaLinear(config.hidden_size, self.all_head_size, config.num_attention_heads, dyna_dim=[False, True], bias=config.qkv_bias)
...
class ViTAttention(nn.Module):
    ...
    # New Method to Reorder Heads
    def reorder_heads(self, idx):
        n, a = self.attention.num_attention_heads, self.attention.attention_head_size
        index_base = torch.arange(n*a).reshape(n, a).to(self.attention.query.weight.device)
        idx = idx.to(self.attention.query.weight.device)
        index = index_base[idx].view(-1).contiguous().long()
        
        # index = torch.arange(n*a).reshape(n, a)[idx].view(-1).contiguous().long()
        # index = index.to(self.attention.query.weight.device)
        ...
        reorder_head_matrix(self.attention.query, index)
        reorder_head_matrix(self.attention.key, index)
        reorder_head_matrix(self.attention.value, index)
